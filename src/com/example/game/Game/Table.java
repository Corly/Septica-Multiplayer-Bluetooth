...
public class Table
{
	...
	{
		if (mCount != 0 && mCount % mNumberOfPlayers == 0 && !mCards[0].getNumber().contentEquals(card.getNumber()) && !card.getNumber().contentEquals("7"))
		{
			return false;
		}
	...
	public HandWinner checkHandWinner(int whichPlayerWasFirst)
	{
		...
		{
			if (mCards[i].getNumber().contentEquals(firstCardName) || mCards[i].getNumber().contentEquals("7"))
			{
				whatCardWon = i;
				break;
			}
		...
		for (int i = 0; i < mCount; i++)
		{
			if (mCards[i].getNumber().contentEquals("a") || mCards[i].getNumber().contentEquals("10"))
			{
				numberOfPoints++;
			}
		}
